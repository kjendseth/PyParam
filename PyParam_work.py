...
IMPORTANT INFORMATION:
    ...
    Set the PATHS to ORCA and AMBER programs below.
...
orca_path = "/Users/asmunroh/Library/Orca420/"
antechamber_path = "/Users/asmunroh/Prog/amber/amber18/bin/"
...
parser = argparse.ArgumentParser(description='RESP calculation from Orac output')

parser.add_argument(
    '-resp',
    default="off",
    help='Turn RESP calculation on? Can be time consuming (default: off)'
)

parser.add_argument(
    '-force',
    default="on",
    help='Turn RESP calculation on? Can be time consuming (default: on)'
)

parser.add_argument(
    '-p',
    default=150,
    help='provide an number of ESP points (default: 150)'
)

parser.add_argument(
    '-l',
    default=10,
    help='provide an number of ESP layers (default: 10)'
)

parser.add_argument(
    '-o',
    default="PyParam_RESP.mol2",
    help='Provide name of out files (default: PyParam_RESP.mol2)'
)

parser.add_argument(
    '-at',
    default="None",
    help='Provide atom name of special interest in frcmod file (default: none, e.g. CT, )'
)
...
#Function to find system charge from orac input file
def find_charge(orca_input_file):
    with open(orca_input_file, 'r') as file:
      data = file.read().split()
      charge = int(data[data.index("xyz") + 1])
    return charge

#Function to find system multiplicity from orac input file
def find_mult(orca_input_file):
    with open(orca_input_file, 'r') as file:
      data = file.read().split()
      mult = int(data[data.index("xyz") + 2])
    return mult
...
def gen_esp_points(x, y, z, npoints):
    ...
    for h in range(0, len(shell_list)):
        ...
        for i in range(0, len(x)):
            shell = np.array(sunflower_sphere(samples= int(npoints * (shell_list[h] ** 2 / shell_list[0] ** 2)))) * shell_list[h] * 0.01 * ang_to_au * np.float(at_radii[elements.index(atoms[i])])
        ...
        for i in range(0, len(atoms)):
            for j in range(0, len(molecule_shell)):
                if (np.linalg.norm(np.array([x[i] * ang_to_au - molecule_shell[j,0], y[i] * ang_to_au - molecule_shell[j,1], z[i] * ang_to_au - molecule_shell[j,2]])) < (0.999 * shell_list[h] * 0.01 * ang_to_au * np.float(at_radii[elements.index(atoms[i])]))):
...
def read_hessian(natoms):    
    ...
    df_hess = pd.read_csv('orca.hess', delim_whitespace=True, skiprows=14, nrows=(3 * natoms * n_chunks + n_chunks) - 1)
    arr = df_hess.values
